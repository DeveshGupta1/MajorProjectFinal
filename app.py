...
def measure_dim(loc):
	...
	#Image ke Edges/Boundary nikali
	edged = cv2.Canny(gray, 50, 100)

	#Image ko acche se saaf kardiya
	...
	for c in cnts:
	    ...
	    # plt.axis('off')
	    # plt.savefig(folder+str(var)+'.png')
	    # var=var+1
...
def uploaded_file(filename):
	...
	dims/=10000
	return render_template('calculator.html', val=1, dims=dims,filename='images/modified/'+filename)


# All routes beyond this
@app.route('/')
def home():
	return render_template('index.html', title='Rooftop Rainwater Harvesting')
